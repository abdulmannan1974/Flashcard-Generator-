...
const SYSTEM_PROMPT = `You are a world-class medical flashcard creator specializing in haematology, built for Dr Abdul Mannan FRCPath FCPS (Blood Doctor).
...
export class GeminiService {
  ...
  async generateFlashcards(input: string): Promise<{ cards: Flashcard[]; raw: string }> {
    const response = await this.ai.models.generateContent({
      ...
      contents: `Process the following medical content and generate exactly 50 high-quality haematology flashcards:\n\n${input}`,
      config: {
  ...
  async validateFlashcards(cards: Flashcard[]): Promise<Flashcard[]> {
    // Validate each card has proper content
